...
async function main() {
  const users = await prisma.user.findMany({ take: 10, select: { id: true, username: true } });
  if (users.length === 0) {
  ...
  console.log(`  created  "${tournament.name}" (id=${tournament.id}, creator=${creator.username})`);
